...
public partial class MainWindow : Window
{
    ...
    private readonly DispatcherTimer _restoreBackupPopupOpenTimer;
    private readonly DispatcherTimer _restoreBackupPopupCloseTimer;

    public MainWindow()
    {
        InitializeComponent();

        _restoreBackupPopupOpenTimer = new DispatcherTimer
        {
            Interval = TimeSpan.FromMilliseconds(50),
        };
        _restoreBackupPopupOpenTimer.Tick += RestoreBackupPopupOpenTimer_OnTick;

        _restoreBackupPopupCloseTimer = new DispatcherTimer
        {
            Interval = TimeSpan.FromMilliseconds(120),
        };
        _restoreBackupPopupCloseTimer.Tick += RestoreBackupPopupCloseTimer_OnTick;
    ...
    {
        RestoreBackupPopup.IsOpen = false;
        _restoreBackupPopupOpenTimer.Stop();
        _restoreBackupPopupCloseTimer.Stop();
    ...
    private void AutoOptimizeAllButton_OnMouseEnter(object sender, MouseEventArgs e)
    {
        _restoreBackupPopupCloseTimer.Stop();

        if (RestoreBackupPopup.IsOpen)
            return;

        _restoreBackupPopupOpenTimer.Stop();
        _restoreBackupPopupOpenTimer.Start();
    }

    private void AutoOptimizeAllButton_OnMouseLeave(object sender, MouseEventArgs e)
    {
        _restoreBackupPopupOpenTimer.Stop();
        StartRestoreBackupPopupCloseTimer();
    }

    private void RestoreBackupPopupRoot_OnMouseEnter(object sender, MouseEventArgs e)
    {
        _restoreBackupPopupOpenTimer.Stop();
        _restoreBackupPopupCloseTimer.Stop();
    }

    private void RestoreBackupPopupRoot_OnMouseLeave(object sender, MouseEventArgs e)
    {
        StartRestoreBackupPopupCloseTimer();
    }

    private void StartRestoreBackupPopupCloseTimer()
    {
        if (!RestoreBackupPopup.IsOpen)
            return;

        _restoreBackupPopupCloseTimer.Stop();
        _restoreBackupPopupCloseTimer.Start();
    }

    private void RestoreBackupPopupOpenTimer_OnTick(object? sender, EventArgs e)
    {
        _restoreBackupPopupOpenTimer.Stop();

        if (!AutoOptimizeAllButton.IsMouseOver || RestoreBackupPopup.IsOpen)
            return;

        RestoreBackupPopup.IsOpen = true;
        _restoreBackupPopupCloseTimer.Stop();
    }

    private void RestoreBackupPopupCloseTimer_OnTick(object? sender, EventArgs e)
    {
        if (AutoOptimizeAllButton.IsMouseOver || RestoreBackupPopupRoot.IsMouseOver)
            return;
